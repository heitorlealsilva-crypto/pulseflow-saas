...
const csvCell=value=>{let text=String(value??'');if(/^[=+\-@]/.test(text.trimStart()))text="'"+text;return `"${text.replace(/"/g,'""')}"`};
export function leadsToCsv(leads=[]){const columns=[['nome','name'],['whatsapp','phone'],['origem','origin'],['interesse','interest'],['pipeline','board'],['etapa','stage'],['produto','product'],['valor_contrato','contractValue'],['nicho','niche'],['faturamento','revenue'],['necessidade','needs'],['notas','notes']];return '\ufeff'+[columns.map(([label])=>csvCell(label)).join(';'),...leads.map(lead=>columns.map(([,key])=>csvCell(lead?.[key])).join(';'))].join('\r\n')}
const csvHeader=value=>String(value||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().trim().replace(/[^a-z0-9]+/g,'_').replace(/^_|_$/g,'');
function csvRows(text,delimiter){const rows=[];let row=[],cell='',quoted=false;for(let i=0;i<text.length;i++){const char=text[i];if(quoted){if(char==='"'&&text[i+1]==='"'){cell+='"';i++}else if(char==='"')quoted=false;else cell+=char}else if(char==='"')quoted=true;else if(char===delimiter){row.push(cell);cell=''}else if(char==='\n'){row.push(cell.replace(/\r$/,''));rows.push(row);row=[];cell=''}else cell+=char}if(cell||row.length){row.push(cell.replace(/\r$/,''));rows.push(row)}return rows}
const importedNumber=value=>{const raw=String(value||'').trim().replace(/^'(?=[=+\-@])/,'');return phoneNumber(raw)};
const importedMoney=value=>{let raw=String(value||'').replace(/[^\d,.-]/g,'');if(raw.includes(',')&&raw.includes('.'))raw=raw.replace(/\./g,'').replace(',','.');else raw=raw.replace(',','.');const result=Number(raw);return Number.isFinite(result)&&result>=0?result:0};
export function parseLeadCsv(text,workspace=freshWorkspace()){
 if(typeof text!=='string'||!text.trim())return {leads:[],skipped:0,errors:['Arquivo vazio.']};
 const first=(text.replace(/^\ufeff/,'').split(/\r?\n/,1)[0]||''),delimiter=(first.match(/;/g)||[]).length>=(first.match(/,/g)||[]).length?';':',';
 const rows=csvRows(text.replace(/^\ufeff/,''),delimiter).filter(row=>row.some(cell=>cell.trim()));if(rows.length<2)return {leads:[],skipped:0,errors:['Inclua o cabeçalho e pelo menos um contato.']};
 const aliases={nome:'name',name:'name',whatsapp:'phone',telefone:'phone',phone:'phone',origem:'origin',source:'origin',interesse:'interest',interest:'interest',pipeline:'board',board:'board',etapa:'stage',stage:'stage',produto:'product',product:'product',valor_contrato:'contractValue',contract_value:'contractValue',nicho:'niche',faturamento:'revenue',necessidade:'needs',notes:'notes',notas:'notes'};
 const headers=rows[0].map(value=>aliases[csvHeader(value)]||''),seen=new Set((workspace.leads||[]).map(lead=>phoneNumber(lead.phone)).filter(Boolean)),leads=[],errors=[];let skipped=0;
 if(!headers.includes('name')||!headers.includes('phone'))return {leads:[],skipped:0,errors:['O CSV precisa das colunas nome e whatsapp.']};
 for(const [offset,row] of rows.slice(1,5001).entries()){const item={};headers.forEach((key,index)=>{if(key)item[key]=String(row[index]??'').trim().slice(0,key==='notes'?10000:1000)});const phone=importedNumber(item.phone);if(!item.name||!phone){errors.push(`Linha ${offset+2}: nome ou WhatsApp inválido.`);skipped++;continue}if(seen.has(phone)){skipped++;continue}seen.add(phone);const board=BOARDS.find(value=>csvHeader(value)===csvHeader(item.board))||'Principal',columns=board==='Pós-venda'?workspace.postSaleColumns:workspace.columns,stage=columns.find(column=>csvHeader(column.id)===csvHeader(item.stage)||csvHeader(column.name)===csvHeader(item.stage))?.id||columns[0]?.id||'new';leads.push({...item,phone,board,stage,interest:['Alta','Média','Baixa'].includes(item.interest)?item.interest:'Média',contractValue:importedMoney(item.contractValue),consentConfirmed:false,optOut:false,messages:[],calls:[],entered:Date.now()})}
