...
(async()=>{
 ...
 const teammate=await call('login',{email:teammateEmail,password:teammatePassword});assert.equal(teammate.status,200);assert.equal(teammate.data.user.organization_id,org);assert.equal((await call('team&organization_id='+org,undefined,teammate.cookie)).status,200);assert.equal((await call('team-user',{organization_id:org,operation:'create',name:'Inválido',email:'invalid@example.test',password:teammatePassword},teammate.cookie)).status,403);
